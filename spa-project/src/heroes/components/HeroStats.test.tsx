...
describe('HeroStats', () => {
  ...
  test('should change the percentage of favorites when a hero is added to favorites', async () => {
    localStorage.setItem('favorites', JSON.stringify([mockHero]));

    renderHeroStats(mockSummaryData);

    const favoritePercentageElement = await screen.findByTestId('favorite-percentage');
    expect(favoritePercentageElement.innerHTML).toContain('4.00%');

    const favoriteCountElement = await screen.findByTestId('favorite-count');
